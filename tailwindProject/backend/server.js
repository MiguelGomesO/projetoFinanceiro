...
app.patch("/usuarios/:id/darkmode", async (req, res) => {
    ...
    if (darkMode === undefined){
        return res.status(400).json({ msg: "Valor de dark_mode é obrigatório"});
    }
...
app.get("/transacoes/:usuarioID", async (req, res) => {
    ...
    try {
        const resultado = await pool.query("SELECT * FROM transacoes WHERE usuario_id = $1 ORDER BY data DESC", [usuarioID]);

        res.json(resultado.rows);
    } catch (err) {
        res.status(500).json({ msg: "Erroa ao buscar transações", erro: err.message });
    }
});
